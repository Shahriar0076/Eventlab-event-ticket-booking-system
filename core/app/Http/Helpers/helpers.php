...
function menuActive($routeName, $type = null, $param = null)
{
    ...
    elseif ($type == 4) $class = 'style="display: block;"';
    elseif ($type == 5) $class = 'link-active';
    else $class = 'active';
...
{
    return fileManager()->removeFile($path . '/' . $filename);
}
...
function isMapsIframeHtml($string)
{
    if (preg_match('/<iframe.*src="https:\/\/www\.google\.com\/maps\/embed\?.*<\/iframe>/', $string)) {
        return true;
    } else {
        return false;
    }
...
function authOrganizerId()
{
    return authOrganizer()?->id;
}
...
function returnBack($message, $type = "error", $withInput = false, $route = null, $routeId = 0)
{
    $notify[] = [$type, $message];
    ...
    if ($withInput) {
        ...
        if ($routeId != 0) {
            return to_route($route, $routeId)->withNotify($notify);
        }
...
function ticketDownloadUrl($orderId)
{
    return '<a href="' . route('event.ticket.download', encrypt($orderId)) . '" target="_blank">' . trans('Download Ticket') . '</a>';
}

function getEventRoute($event)
{
    if ($event->step < 5) {
        return route('organizer.event.overview', $event->id);
    } else {
        return route('event.details', $event->slug);
    }
}
